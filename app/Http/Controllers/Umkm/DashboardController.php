...
class DashboardController extends Controller
{
    ...
    public function tambah_eventual(Request $request){
      ...
      //  $kegiatan = Kegiatan::select('tema')->where('id',$data['kegiatan_id'] )->first();
             foreach ($admins as $admin) {
                 $notif = Notifikasi::create(['nama'=>'Pengajuan Eventual','pesan'=>'UMKM'. $request->user()->profil->nama_lengkap. ' mengajukan untuk meeting di kegiatan '.$data->kegiatan->tema,'user_id'=>$admin->user_id,'redirect'=> '/admin/dashboard/kegiatan'. $data['kegiatan_id'].'/eventual','tanggal'=>now()]);
  ...
  public function beranda(){
    ...
    public function detail_materi($page,$page2)
    {
        ...
        $data =Elearning::with(['bab'])
                ->where('elearning.id','=',$page)
                ->first(); 
        
        $next =DB::table('bab')->where('bab','=',intval($page2)+1)->first();
    ...
    public function elearning($page){
         $kegiatan = Kegiatan::select('kegiatan.tema','kegiatan.id as kegiatan_id')
