<template>
    ...
    <section  v-else >
       <div class="row">
        ...
        <MultiSearch :lazy="['investor','investor_all']"  :url="`/admin/investasi/${bisnis.id}/investor?page=1`"  />
        ...
        <table class="table mt-3">
          ...
          <tbody>
             <tr  v-for="(index,no) in investor_all.data">
              ...
              <td>{{index.no_hp}}</td>
              <td >
               <button v-if="investor.includes(index.id)" @click="this.toggleDeleteModal(id_delete=index.id)"  class="btn btn-semantic-error-4 text-neutral-white">
                  <img src="../../../assets/icons/icon_delete.png" alt="delete icon">
                  Keluarkan dari bisnis
                </button  >
                 <button v-else @click="this.submitTambahInvestor(investor_id=index.id)" class="btn btn-semantic-success-4  text-neutral-white">
      ...
      <div v-if="this.investor_all !=null " class="d-flex justify-content-center mt-4">
          <Pagination :lazy="['investor','investor_all']" :links="investor_all.links"/>
      </div>
    </section>
  </div>
      </div>
<RemoveInvestorLayout pesan="Anda yakin ingin mengeluarkan investor ini" :popup="this.deletePopup" :itemDelete="this.itemDelete" url="/admin/investasi/hapus_investor" @toggleClose="toggleDelete()" />   
<RemoveInvestorLayoutModal :lazy="['investor','investor_all']" pesan="Anda yakin ingin mengeluarkan investor ini" :popup="this.deletePopupModal" :itemDelete="this.itemDelete" url="/admin/investasi/hapus_investor" @toggleClose="toggleDeleteModal()" />  
 <Layout section="investor" :title="bisnis.name" :link="bisnis.id">
    <ul class="tabs mt-4">
      <li class="active" >
    ...
    <MultiSearch :url="`/admin/investasi/${bisnis.id}/investor?page=1`"/>
...
export default{
    ...
    methods:{
       ...
       toggleTambahInvestor(){
        ...
        Inertia.visit('/admin/investasi/'+this.bisnis_id+'/investor?page=1',{ only: ['investor','investor_all'],preserveState:true})
         

       },
       toggleTutup(){
        this.popupTambahInvestor = !this.popupTambahInvestor
        Inertia.visit('/admin/investasi/'+this.bisnis.id+'/investor?page=1',{preserveState:true})
        
       },
      submitTambahInvestor(investor_id){
        Inertia.post('/admin/investasi/tambah_investor'
          ,{investor_id:investor_id,bisnis_id:this.bisnis.id}
