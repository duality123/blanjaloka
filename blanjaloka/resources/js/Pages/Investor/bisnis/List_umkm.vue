<template>
 ...
 <Layout state="daftar_umkm" :title="bisnis.name" :link="bisnis.id">
                ...
                <div class="d-flex flex-column flex-lg-row gap-2 bg-primary-blue-1 rounded px-3 py-2 mt-4 mb-4">
                  ...
                  <p class="fw-semibold text-primary-blue-6 mb-0">
                    Disini adalah daftar UMKM yang ikut masuk ke funding investasi ini.
                  </p>
                </div>

         <div class=" d-flex justify-content-start">
        <MultiSearch :url="`/umkm/dashboard/funding/daftar_umkm/${bisnis.id}?page=1`" />
          ...
          <tbody>
             <tr  v-for="(index,no) in items.data">
              ...
              <td><img class="imgtd" :src="`${$page.props.asset_url}/${index.foto_profil}`"/></td>
               <td>
                <button v-if="loading" @click="togglePopup(index.umkm_id)" class="btn btn-primary-blue-6 me-2 px-3 text-neutral-white cursor-pointer"><div class="spinner-border text-primary"></div></button>
                <button v-else @click="togglePopup(index.umkm_id)" class="btn btn-primary-blue-6 me-2 px-3 text-neutral-white cursor-pointer">{{index.nama_lengkap}}</button></td>
...
export default{
  ...
  togglePopup(investor_id){
    ...
    Inertia.get('/investor/dashboard/bisnisku/daftar_umkm/'+this.bisnis.id,{id:investor_id},{ only: ['umkm'],preserveState:true})
  },
