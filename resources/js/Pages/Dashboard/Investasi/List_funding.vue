<template>
    <RemoveKegiatanLayout pesan="anda ingin menghapus track record ini ?" :popup="deletePopup" :itemDelete="itemDelete" url="/admin/investasi/hapus_funding_umkm" @toggleClose="switchClose()" />
...
<div class="col-lg-8" v-if="popupTambah" >
      ...
      <div class="mb-4">
         <label for="provinsi" class="form-label text-neutral-gray-5">Masukkan Uang Keluar (funding)</label>
         <input type="number" class="form-control" v-model="form.uang_keluar" id="provinsi" placeholder="Masukkan uang keluar" >
           <small class="text-danger" v-if="form.errors.uang_keluar">{{ form.errors.uang_keluar }}</small>
      </div>
       <div class="mb-4">
...
<div class="col-lg-8" v-if="modalStat">
  ...
  <div class="card-header">Total Uang Masuk</div>
  <div class="card-body statData">
    <h1 >Rp. {{statistik.fundingMasukTotal}}</h1>
  </div>
</div>
      </div>
      <div class="col-lg-6 mb-4 ">
  ...
  <div class="card-body statData">
    <h1 >Rp. {{statistik.fundingKeluarTotal}}</h1>
  </div>
</div>
      </div>
      <div class="col-lg-6 mb-4 ">
        <div class="card text-neutral-white bg-primary-blue-6 mb-3 mark" >
  <div class="card-header">Kontribusi UMKM terhadap Penghasilan</div>
  <div class="card-body statData">
 ...
 <Layout section="funding" :title="bisnis.name" :link="bisnis.id">
    <ul class="tabs mt-4">
      <li class="active" >
      		<h3>Funding Masuk dan Keluar UMKM</h3>
      </li>
    </ul>
    <section>
    <div class="row">
         <div class="col d-flex justify-content-start">
      <Search :url="`/admin/investasi/${bisnis.id}/detail_funding/${currentUser}?page=1`" judul="Cari Track Record Berdasarkan Tanggal / Nilai" /> 
        ...
        <table class="table mt-3">
          <thead class="table-primary-blue-4">
            <tr>
              ...
              <th scope="col">Funding Keluar</th>
              <th scope="col">Funding Masuk</th>
          ...
          <tbody>
           <tr v-for="(index,no) in items.data">
              <th scope="row">{{++no}}</th>
              <td>{{index.tanggal}}</td>
              <td>Rp. {{index.uang_keluar}}</td>
              <td>Rp. {{index.uang_masuk}}</td>
                <td> <a :href="`${$page.props.asset_url}/${index.bukti}`" class="btn btn-primary-blue-6 text-neutral-white py-2">
        Lihat Bukti
        </a></td>
        <td class="d-flex flex-column flex-lg-row justify-content-center gap-4">
                <button @click="this.togglePopupEdit(moneyIn=index.uang_masuk,moneyOut=index.uang_keluar,buktiIn=index.bukti)" :href="`/admin/kegiatan/${index.id}/edit`" class="btn btn-semantic-success-4 text-neutral-white">
...
export default{
    data(){
        return{
          ...
          currentUser:document.location.pathname.split('/')[5],
          modalStat:false,
          chartData:{},
          chartOptions:{
              responsive: true,
  ...
  setup(props){
    ...
    computed:{
      chartData(){
         if(this.statistik){
          ...
          datasets: [{
            label: 'Uang Masuk ( Rp )',
            data: this.statistik.fundingMasuk.nilai,
            fill: false,
            borderColor: '#398ab9',
            tension: 0.1
          },
          {
            ...
            data:  this.statistik.fundingKeluar.nilai,
            fill: false,
            borderColor: '#d64036',
            tension: 0.1
          }]
    ...
    methods:{
      ...
      togglePopupTambah(){
       ...
       togglePopupStat(){
        this.modalStat= true
        Inertia.get('/admin/investasi/'+this.bisnis.id+'/detail_funding/'+this.umkm+'?page=1',{},{ only: ['statistik'],preserveState:true})


      },
      togglePopupStatClose(){
        this.modalStat= false
        Inertia.get('/admin/investasi/'+this.bisnis.id+'/detail_funding/'+this.umkm+'?page=1',{},{preserveState:true})
      ...
      inputDokumen(event){
        if(event.target.files[0].type == 'application/pdf' || event.target.files[0].type == 'application/msword'){
            ...
            this.fileReviewer = this.form.bukti.name
