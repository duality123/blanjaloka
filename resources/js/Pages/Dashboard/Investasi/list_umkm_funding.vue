<template>
 ...
 <Layout section="funding" :title="bisnis.name" :link="bisnis.id">
   <MultiSearch :url="`/admin/investasi/${bisnis.id}/umkm_funding?page=1`" />
        ...
        <table class="table mt-3">
          ...
          <tbody>
             <tr  v-for="(index,no) in items.data">
              <th scope="row">{{++no}}</th>
              <td><img class="imagetd" :src="`${$page.props.asset_url}/${index.profil.foto_profil}`"/></td>
              <td><Link class="btn btn-primary-blue-6 me-2 px-3 text-neutral-white cursor-pointer" :href="`/detail/profil/umkm/${index.id}/`">{{index.profil.nama_lengkap}}</Link></td>
              <td>{{index.usaha.nama_perusahaan}}</td>
              <td>{{index.produk.kategori_produk}}</td>
              <td>{{index.profil.no_hp}}</td>
                <td>
                <td>  <Link class="btn btn-primary-blue-6 text-neutral-white" :href="`/admin/investasi/${bisnis.id}/detail_funding/${index.id}`">Lihat Funding</Link></td>
              </td>
            </tr>
          </tbody>
        </table>
