    ...
    <Layout section="eventual" :title="kegiatan.tema" :link="kegiatan.id">
        ...
        <table class="table mt-3">
          ...
          <tbody>
             <tr v-for="(index,no) in items">
              <th scope="row">{{++no}}</th>
              <td>{{index.hari_tanggal_bulan}}</td>
      ...
      <div class="d-flex justify-content-center mt-4">
        <ul class="pagination">
          ...
          <li v-if="first">
              <Link :href="`${$page.url}/${first}`">{{first}}</Link>
          </li>
          <li v-if="prevBlok">
              <Link :href="`${$page.url}/{prevBlok}`">...</Link>
          </li>
          <div v-for="num in paginationNums">
          <li :class="[ currentPage == num ? 'active':'']">
              <Link :href="`${$page.url}/${num}`">{{num}}</Link>
          </li>
        </div>
        <li v-if="nextBlok">
          <Link :href="`${$page.url}/${nextBlok}`">...</Link>
        </li>
        <li v-if="last">
            <Link :href="`${$page.url}/${last}`">{{last}}</Link>
        </li>
          <li v-if="next">
