  ...
  <BaseLayout page="profil">
        <div class="card-body">
                ...
                <input type="file" class="custom-file-input2" style="width: 9rem;">
                 ...
                 <small class="text-danger text-center" v-if="this.form.errors.foto_profil">{{this.form.errors.foto_profil}}</small>
...
export default{
  ...
  setup () {
    const form = useForm({
      foto_profil: usePage().props.value.auth.profil['foto_profil'],
    })
    ...
    changePicture(event){
        if(event.target.files[0].type == 'image/jpeg' || event.target.files[0].type == 'image/png'){
            ...
            if(event.target.files[0].size >= 5000000){
              this.form.errors.foto_profil = "Size foto anda lebih dari 5MB !" 
              return
            }
        ...
        this.form.foto_profil = event.target.files[0];
         ...
         this.form.errors.foto_profil = "File yang anda upload bukanlah gambar !" 
         return
      }
    },
  submit() {
      if (this.process || !this.$page.props.auth.profileComplete ){
        this.form.post('/investor/profil/proses/wizard_pertama');
      }
      else{
        Inertia.get('/investor/profil/2');
    }
   } 
