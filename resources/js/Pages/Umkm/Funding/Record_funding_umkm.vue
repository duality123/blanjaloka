<template>
 ...
 <Layout state="fundingku" :title="bisnis.name" :link="bisnis.id">
                ...
                <div class="d-flex flex-column flex-lg-row gap-2 bg-primary-blue-1 rounded px-3 py-2 mt-4 mb-4">
                  ...
                  <p class="fw-semibold text-primary-blue-6 mb-0">
                    Disini adalah record funding anda.
                  </p>
                </div>
        <div class="row">
        <div class="col d-flex justify-content-start">
        <Search :url="`/umkm/dashboard/funding/data_fundingku/${bisnis.id}?page=1`" judul="Cari berdasarkan tanggal atau nilai Keluar masuk funding" />   
      </div>
        ...
        <table class="table mt-3">
          ...
          <tbody>
           <tr v-for="(index,no) in items.data">
              <th scope="row">{{++no}}</th>
              <td>{{index.tanggal}}</td>
              <td>{{index.uang_keluar}}</td>
              <td>{{index.uang_masuk}}</td>
...
export default{
  ...
  methods:{
    submit(){
     ...
     togglePopupStat(){
        this.modalStat= true
         Inertia.get('/umkm/dashboard/funding/'+this.bisnis.id+'/data_fundingku/?page=1',{},{only: ['statistik'],preserveState:true})


      },
     togglePopupStatClose(){
        this.modalStat= false
        Inertia.get('/umkm/dashboard/funding/'+this.bisnis.id+'/data_fundingku/?page=1',{},{only: ['statistik'],preserveState:false})
